...
## 6. PacketSession 구현  
패킷 앞에 헤더를 약식으로 구현해놓고 패킷을 전송할 때 이 헤더를 기입하고, 그 정보를 바탕으로 받은 패킷의 데이터를 추출하는 코드의 기반을 구현한다.  

보안에 대한 내용도 약간 등장하는데, 이 패킷 헤더를 조작하는 공격자들이 있을 수 있기 때문이다. 때문에 어떤 헤더를 받았을 때 이 헤더 내용이 뭔가 의심된다면 해당 대상을 킥한다거나 등의 보안조치를 구현할 수 있다.  
즉, 패킷 헤더는 완전히 신뢰할 수 없다.  

PacketSession 참조  
(강의 뒷부분에 현 구조에서 발생할 수 있는 문제 및 멀티스레드 환경에서의 디버깅에 대한 예시가 나오므로 꼭 다시 살펴보자)  
